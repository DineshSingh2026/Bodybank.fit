...
const EVENT_META = {
  ...
  MEMBERSHIP_DIGEST: { priority: PRIORITY.IMPORTANT, dedup: 0 },
  ...
  SMART_SCALE_UPLOADED: { priority: PRIORITY.IMPORTANT, dedup: 0 },
...
const FORMATTERS = {
  ...
  MEMBERSHIP_DIGEST: (p) => ['☎️ Membership Call Queue', `⏳ Expiring in ≤2 days: ${s(p.expiring_count)}`, `🔴 Just expired: ${s(p.expired_count)}`, ...(Array.isArray(p.lines) && p.lines.length ? ['', '👇 Who to call:', ...p.lines] : ['', '✅ Nobody expiring right now']), `⏰ ${ts()}`],
